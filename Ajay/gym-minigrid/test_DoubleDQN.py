...
def main(gamma=0.99, lr=1e-3, min_episodes=20, eps=1, eps_decay=0.995, eps_min=0.01, update_step=10, batch_size=64, update_repeats=50,
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            print("Check state:",i, episode, action, reward, loc)
    ...
    plt.savefig(env_name+"_rewards_DoubleDQN_MaxMemory.png")
    plt.close()
    plt.plot(eps_logs)
    plt.xlabel('epochs')
    plt.ylabel('epsilon')
    plt.savefig(env_name+"_epsilon_DoubleDQN_MaxMemory.png")
    plt.close()
    plt.plot(steps)
    plt.xlabel('episodes')
    plt.ylabel('steps')
    plt.title('Steps required for termination')
    plt.savefig(env_name+"_steps_DoubleDQN_MaxMemory.png")
    plt.close()
    print("Performance:",performance)
    print("Epsilon:", eps_logs)
    print("Steps: ", steps)
    MODEL_PATH = 'DoubleDQN_MaxMemory_Q1_'+str(env_name)+'_'+str(num_episodes)+'.pt'
    torch.save(Q_1, MODEL_PATH)
    MODEL_PATH = 'DoubleDQN_MaxMemory_Q2_' + str(env_name) + '_' + str(num_episodes) + '.pt'
